  ...
  return (
    <Container variant="sidebar">
      ...
      <VStack align={'start'} flex={1} padding={4}>
        <Text fontSize={'sm'} color={'text.pale'}>Menu</Text>
        {Sidebar_Item.menu.map(el => <SidebarItem key={el.title} data={el} />)}
        <Divider/>
        {Sidebar_Item.settings.map(el => <SidebarItem key={el.title} data={el} />)}
      </VStack>
